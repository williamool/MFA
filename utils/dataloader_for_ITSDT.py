...
class seqDataset(Dataset):
    def __init__(self, dataset_path, image_size, num_frame=5 ,type='train'):
        ...
        ###############################
        description = pickle.load(open('D:/Github/MoPKL/emb_train_ITSDT.pkl', 'rb'))
        ...
        relation = pickle.load(open('D:/Github/MoPKL/motion_relation_ITSDT.pkl', 'rb'))
    ...
    def get_caption_data(self, index):
        ...
        # Convert Windows path to Linux path format for matching with cap_idx
        linux_path = file_name.replace('D:/Github/ITSDT/', '/home/public/ITSDT/')
        
        try:
            cap_index = self.cap_idx.index(linux_path)
        except ValueError:
            # If exact match not found, try to find a similar path
            print(f"Warning: File {file_name} not found in cap_idx. Trying alternative matching...")
            # Try to find by filename only
            filename = file_name.split('/')[-1]
            cap_index = None
            for i, cap_path in enumerate(self.cap_idx):
                if cap_path.endswith(filename):
                    cap_index = i
                    break
            
            if cap_index is None:
                raise ValueError(f"File {file_name} (converted to {linux_path}) not found in cap_idx")
